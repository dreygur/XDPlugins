...
async function createDialog(selection) {
    ...
    function overflow(mainkey, key) {
        ...
        mdplCC = materialcolors[mainkey][key];
        ...
        mdplAF.onclick = _ => {
            ...
            selection.items.forEach(element => {
                if (validelements.includes(element.constructor.name)) {
                    console.log("try to fill :", element.constructor.name, materialcolors[mainkey][key]);
    ...
    try {
        ...
        setTimeout(() => {
            ...
            Object.keys(materialcolors)
                ...
                Object.keys(materialcolors[keymain])
                    ...
                    if (materialcolors[keymain][key] == mdplCC)
        ...
        if (selection.items.length == 1 && selection.items[0].fillEnabled && selection.items[0].fill.value) {
            ...
            Object.keys(materialcolors)
                ...
                Object.keys(materialcolors[keymain])
                    .forEach(key => {
                    if (fillcolor == materialcolors[keymain][key]) {
                        console.log(fillcolor, materialcolors[keymain][key], fillcolor == materialcolors[keymain][key]);
                        overflow(keymain, key);
                        notfound = false;
                    }
                });
